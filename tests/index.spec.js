...
test("O CPF 386.995.390-02 deve ser válido", () => {
  expect(verificarCPF("386.995.390-02")).toBe(false);
});
test("O CPF 773.857.990-00 deve ser válido", () => {
  expect(verificarCPF("773.857.990-00")).toBe(false);
});
test("O CPF 124.591.790-06 deve ser válido", () => {
  expect(verificarCPF("124.591.790-06")).toBe(false);
});
test("O CPF 049.339.270-06 deve ser válido", () => {
  expect(verificarCPF("049.339.270-06")).toBe(false);
});
test("O CPF 568.466.830-54 deve ser válido", () => {
  expect(verificarCPF("568.466.830-54")).toBe(false);
});
